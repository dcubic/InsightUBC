...
export default class Server {
    ...
    public start(): Promise<boolean> {
        ...
        return new Promise(function (fulfill, reject) {
            try {
                ...
                // http://localhost:4321/dataset/courses/DatasetKind.Courses
                that.rest.put("/dataset/:id/:kind", Server.putDataset);

                // http://localhost:4321/dataset/courses
    ...
    private static putDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        const zipBuffer: Buffer = req.body;
        const kind: InsightDatasetKind = req.params.kind;
        try {
            const zipBase64 = zipBuffer.toString("base64");
            Server.insightFacade.addDataset(id, zipBase64, kind).then((response) => {
                Log.info("Server::echo(..) - responding " + 200);
                res.json(200, {result: response});
            }).catch((err) => {
                Log.error("Server::echo(..) - responding 400");
